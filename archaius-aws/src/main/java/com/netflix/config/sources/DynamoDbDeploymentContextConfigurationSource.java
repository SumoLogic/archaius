 ...
 * Rather than polling dynamo itself this class reads a cache that you have to setup separately. If you used a combined configuration you can cascade these
 * sources to so that certain contexts override others.
 */
public class DynamoDbDeploymentContextConfigurationSource implements PolledConfigurationSource {
   ...
   * @param tableCache
   * @param contextKey
   *          - Null can be used as a wild card to match the absence of a context key for the property. The result would be that this source with a null key
   *          would pick up "global" properties.
   */
  ...
  public PollResult poll(boolean initial, Object checkPoint) throws Exception {
    ...
    for (PropertyWithDeploymentContext prop : tableCache.getProperties()) {
      ...
      } else if (prop.getContextKey() == contextKey && prop.getContextValue().equalsIgnoreCase(deploymentContext.getValue(contextKey))) {
        map.put(prop.getPropertyName(), prop.getPropertyValue());
      }
    }
