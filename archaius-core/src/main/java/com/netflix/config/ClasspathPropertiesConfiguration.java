 ...
 * 
 * Assuming your application utilizes many modules (.jar files) and need Properties support, this class provides a convention based approach to scanning and
 * loading properties from every jar in a classpath from a specific location. All modules are assumed to have properties in a location such as
 * META-INF/conf/configuration.properties. Such resources may be anywhere in the classpath; for example in directories for a unit test or application.
 * 
 * Parameters from such resources are contained in a Configuration whose name is usually the name of the .jar; for example "com.netflix.movieserviceclient.jar".
 * For a resource not in a .jar, the Configuration name is the URL of the directory that contains the resource. However, if the resource contains a property
 * named conf.configName, its value will be used as the Configuration name.
 * <p>
...
public class ClasspathPropertiesConfiguration extends ConcurrentMapConfiguration {
  ...
  /**
   * Sets the name for the property name that defines the name for a bag of properties loaded from a properties resources
   *
   * Default if not set is config.configName Dead code. No longer needed. Deprecate first before removing.
   *
   ...
   *          New relative path of config file
   */
