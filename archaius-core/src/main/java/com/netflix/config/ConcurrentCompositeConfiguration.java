...
/**
 * This class maintains a hierarchy of configurations in a list structure. The order of the list stands for the descending priority of the configurations when a
 * property value is to be determined. For example, if you add Configuration1, and then Configuration2, {@link #getProperty(String)} will return any properties
 * defined by Configuration1. Only if Configuration1 doesn't have the property, then Configuration2 will be checked.
 * </p>
 * There are two internal configurations for properties that are programmatically set:
 * <ul>
 * <li>Configuration to hold any property introduced by {@link #addProperty(String, Object)} or {@link #setProperty(String, Object)} called directly on this
 * class. This configuration will be called "container configuration" as it serves as the container of such properties. By default, this configuration remains
 * at the last of the configurations list. It can be treated as a "base line" configuration that holds hard-coded parameters that can be overridden by any of
 * other configurations added at runtime. You can replace this configuration by your own and change the position of the configuration in the list by calling
 ...
 * configurations on the list. As contrast to container configuration, this configuration is always consulted first in {@link #getProperty(String)}.
 * </ul>
 * 
 * When adding configuration to this class, it is recommended to convert it into {@link ConcurrentMapConfiguration} or ConcurrentCompositeConfiguration using
 * {@link com.netflix.config.util.ConfigurationUtils} to achieve maximal performance and thread safety.
 * 
 * <p>
 * Example:
 * 
...
public class ConcurrentCompositeConfiguration extends ConcurrentMapConfiguration implements AggregatedConfiguration, ConfigurationListener, Cloneable {
  ...
  /**
   * Creates a ConcurrentCompositeConfiguration object with a specified <em>container configuration</em>. This configuration will store any changes made by
   * {@link #setProperty(String, Object)} and {@link #addProperty(String, Object)}.
   *
   * @param containerConfiguration
   *          the configuration to use as container configuration
   */
  ...
  /**
   * Creates a ConcurrentCompositeConfiguration with a specified <em>container configuration</em>, and then adds the given collection of configurations.
   *
   * @param containerConfiguration
  ...
  /**
   * Event listener call back for configuration update events. This method is called whenever one of the contained configurations was modified. This method does
   * nothing.
   *
   * @param event
   *          the update event
  ...
  /**
   * Add a child configuration without a name. Make a call to {@link #addConfiguration(AbstractConfiguration, String)} with the name being null.
   *
   * @param config
  ...
  /**
   * Adds a new child configuration to this configuration with an optional name. The configuration will be added to the end of the list if <em>container
   * configuration</em> has been changed to new one or no longer at the end of the list. Otherwise it will be added in front of the <em>container
   * configuration</em>.
   *
   * @param config
  ...
  /**
   * Adds a child configuration and makes it the <em>container configuration</em>. This means that all future property write operations are executed on this
   * configuration. Note that the current container configuration stays in the list of child configurations at its current position, but it passes its role as
   * container configuration to the new one.
   *
   * @param config
  ...
  /**
   * Removes all child configurations and reinitializes the <em>container configuration</em>. <strong>Attention:</strong> A new container configuration is
   * created; the old one is lost.
   */
  @Override
  ...
  /**
   * Set the property with the <em>container configuration</em>. <b>Warning: </b>{@link #getProperty(String)} on this key may not return the same value set by
   * this method if there is any other configuration that contain the same property and is in front of the <em>container configuration</em> in the
   * configurations list.
   */
  @Override
  ...
  /**
   * Clear the property with the <em>container configuration</em>. <b>Warning: </b>{@link #getProperty(String)} on this key may still return some value if there
   * is any other configuration that contain the same property and is in front of the <em>container configuration</em> in the configurations list.
   */
  ...
  /**
   * Read property from underlying composite. It first checks if the property has been overridden by {@link #setOverrideProperty(String, Object)} and if so
   * return the overriding value. Otherwise, it iterates through the list of sub configurations until it finds one that contains the property and return the
   * value from that sub configuration. It returns null of the property does not exist.
   *
   * @param key
   ...
   *           if concurrent modification happens on any sub configuration when it is iterated to get all the keys
   * 
   */
    ...
    for (Configuration config : configList) {
      for (Iterator<String> it = config.getKeys(); it.hasNext();) {
        try {
          ...
          logger.error("unexpected exception when iterating the keys for configuration " + config + " with name " + getNameForConfiguration(config));
          throw e;
        }
      }
    }
  ...
  /**
   * Returns a set with the names of all configurations contained in this configuration. Of course here are only these configurations listed, for which a name
   * was specified when they were added.
   *
  ...
  /**
   * Returns a copy of this object. This implementation will create a deep clone, i.e. all configurations contained in this composite will also be cloned. This
   * only works if all contained configurations support cloning; otherwise a runtime exception will be thrown. Registered event handlers won't get cloned.
   *
   */
  @Override
  public Object clone() {
  ...
  /**
   * Sets a flag whether added values for string properties should be checked for the list delimiter. This implementation ensures that the container
  ...
  /**
   * Returns the configuration source, in which the specified key is defined. This method will iterate over all existing child configurations and check whether
   * they contain the specified key. The following constellations are possible:
   * <ul>
  ...
  /**
   * Adds the value of a property to the given list. This method is used by {@code getList()} for gathering property values from the child configurations.
   *
   * @param dest
  ...
  /**
   * Set whether sub configurations should propagate events to listeners to this configuration. This is needed if this configuration is used as the
   * configuration source of {@link DynamicPropertyFactory}.
   * 
   * @param propagateEventToParent
   *          value to set
   */
