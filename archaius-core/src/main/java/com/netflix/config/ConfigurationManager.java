 ...
 * <p>
 * During initialization, this class will check system property "archaius.default.configuration.class" and "archaius.default.configuration.factory". If the
 * former is set, it will use the class name to instantiate it using its default no-arg constructor. If the later is set, it will call its static method
 * getInstance(). In both cases, the returned Configuration object will be set as the system wide configuration.
 * 
 * @author awang
 *
 */
public class ConfigurationManager {
  ...
  /**
   * initStack will hold the stack trace at the time of static initialization for ConfigurationManager to help debug where/when ConfigurationManager was
   * created. We do this to help debug issues with Archaius2 where ConfigurationManager is initialized before the bridge has been properly set up
   */
  ...
  /**
   * @return Return the stack trace that triggered static initialization of ConfigurationManager. This information can be used to help debug static
   *         initialization issues with the Archaius2 bridge.
   */
  ...
  /**
   * Install the system wide configuration with the ConfigurationManager. This will also install the configuration with the {@link DynamicPropertyFactory} by
   * calling {@link DynamicPropertyFactory#initWithConfigurationSource(AbstractConfiguration)}. This call can be made only once, otherwise IllegalStateException
   * will be thrown.
   */
  ...
  /**
   * Get the current system wide configuration. If there has not been set, it will return a default {@link ConcurrentCompositeConfiguration} which contains a
   * SystemConfiguration from Apache Commons Configuration and a {@link DynamicURLConfiguration}.
   */
      ...
      if (listeners != null) {
        for (ConfigurationListener listener : listeners) {
          if (listener instanceof ExpandedConfigurationListenerAdapter
            && ((ExpandedConfigurationListenerAdapter) listener).getListener() instanceof DynamicProperty.DynamicPropertyListener) {
   ...
   * 
   * @param path
   *          relative path of the resources
   * @throws IOException
   */
  ...
  /**
   * Load resource configName.properties first. Then load configName-deploymentEnvironment.properties into the system wide configuration. For example, if
   * configName is "application", and deployment environment is "test", this API will first load "application.properties", then load
   * "application-test.properties" to override any property that also exist in "application.properties".
   * 
   * @param configName
   *          prefix of the properties file name.
   * @throws IOException
  ...
  private static synchronized void removeDefaultConfiguration() {
    ...
    for (ConfigurationListener l : listeners) {
      if (l instanceof ExpandedConfigurationListenerAdapter
        && ((ExpandedConfigurationListenerAdapter) l).getListener() instanceof DynamicProperty.DynamicPropertyListener) {
  ...
  public static AbstractConfiguration getConfigFromPropertiesFile(URL startingUrl) throws FileNotFoundException {
    return ConfigurationUtils.getConfigFromPropertiesFile(startingUrl, getLoadedPropertiesURLs(), PROP_NEXT_LOAD, PROP_NEXT_LOAD_NFLX);
  }
