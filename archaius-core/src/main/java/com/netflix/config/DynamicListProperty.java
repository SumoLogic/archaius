...
/**
 * This class delegates to a regex (default is comma) delimited dynamic string property and returns a dynamic list of a generic type which is transformed from
 * string.
 * 
 * @author awang
 */
public abstract class DynamicListProperty<T> implements Property<List<T>> {
   ...
   * <code>Splitter.onPattern(delimiterRegex).omitEmptyStrings().trimResults()</code>. The default list value will be transformed from list of strings after
   * splitting. If defaultValue string is null, the default list value will be an empty list.
   */
  ...
  /**
   * Create the dynamic list property. The guava Splitter used is created as <code>Splitter.onPattern(delimiterRegex).omitEmptyStrings().trimResults()</code>.
   * The default list value will be transformed from list of strings after splitting. If defaultValue string is null, the default list value will be an empty
   * list.
   */
  ...
  /**
   * Create the dynamic list property. The guava Splitter used is created as <code>Splitter.onPattern(delimiterRegex).omitEmptyStrings().trimResults()</code>.
   * The default list value will be taken from the passed in list argument.
   */
  ...
  /**
   * A method invoked when the underlying string property is changed. Default implementation does nothing. Subclass can override this method to receive
   * callback.
   */
