...
/**
 * A cached configuration property value that is automatically updated when the config is changed. The object is fully thread safe, and access is very fast. (In
 * fact, testing indicates that using a DynamicProperty is faster than fetching a System Property.)
 * <p>
 * This class is intended for those situations where the value of a property is fetched many times, and the value may be changed on-the-fly. If the property is
 * being read only once, "normal" access methods should be used. If the property value is fixed, consider just caching the value in a variable.
 * <p>
 * Fetching the cached value is synchronized only on this property, so contention should be negligible. If even that level of overhead is too much for you, you
 * should (a) think real hard about what you are doing, and (b) just cache the property value in a variable and be done with it.
 * <p>
...
public class DynamicProperty {
  ...
  /*
   * Cache update is handled by a single configuration listener, with a static collection holding all defined DynamicProperty objects. It is assumed that
   * DynamicProperty objects are static and never subject to gc, so holding them in the collection does not cause a memory leak.
   */

  private static final ConcurrentHashMap<String, DynamicProperty> ALL_PROPS = new ConcurrentHashMap<String, DynamicProperty>();
  ...
  private abstract class CachedValue<T> {
    ...
    /**
     * Flushes the cached value. Must be called with the {@code lock} variable held by this thread.
     */
    final void flush() {
    ...
    /**
     * Gets the cached value. If the value has not yet been parsed from the string value, parse it now.
     * 
    ...
    /**
     * Gets the cached value. If the value has not yet been parsed from the string value, parse it now. If there is no string value, or there was a parse error,
     * returns the given default value.
     * 
     * @param defaultValue
     *          the value to return if there was a problem
     * @return the parsed value, or the default if there was no string value or a problem during parse
     */
   ...
   * @return a DynamicProperty object that holds the cached value of the configuration property named {@code propName}
   */
  ...
  /**
   * Gets the current value of the property as an Boolean. A property string value of "true", "yes", "on", "t" or "y" produces {@code Boolean.TRUE}. A property
   * string value of "false", "no", "off", "f" or "b" produces {@code Boolean.FALSE}. (The value comparison ignores case.) Any other value will result in an
   * exception.
   *
   * @return the current property value, or null if there is none.
   * @throws IllegalArgumentException
   *           if the property is defined but is not an Boolean
   */
   ...
   *
   * @param defaultValue
   *          the value to return if the property is not defined, or is not of the proper format
   * @return the current property value, or the default value if there is none or the property is not of the proper format
   */
   ...
   * @return the current property value, or null if there is none.
   * @throws IllegalArgumentException
   *           if the property is defined but is not an Integer
   */
   ...
   * @return the current property value, or null if there is none.
   * @throws IllegalArgumentException
   *           if the property is defined but is not a Float
   */
   ...
   * @return the current property value, or null if there is none.
   * @throws IllegalArgumentException
   *           if the property is defined but is not a Long
   */
   ...
   * @return the current property value, or null if there is none.
   * @throws IllegalArgumentException
   *           zif the property is defined but is not the name of a Class
   */
   ...
   *
   * @param defaultValue
   *          the value to return if the property is not defined, or is not the name of a Class
   * @return the current property value, or the default value if there is none or the property is not of the proper format
   */
  ...
  /**
   * A callback object that listens for configuration changes and maintains cached property values.
   */
