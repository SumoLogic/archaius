...
/**
 * A factory that creates instances of dynamic properties and associates them with an underlying configuration or {@link DynamicPropertySupport} where the
 * properties could be changed dynamically at runtime.
 * <p/>
 * It is recommended to initialize this class with a configuration or DynamicPropertySupport before the first call to {@link #getInstance()}. Otherwise, it will
 * be lazily initialized with a {@link ConcurrentCompositeConfiguration}, where a SystemConfiguration and {@link DynamicURLConfiguration} will be added. You can
 * also disable installing the default configuration by setting system property {@value #DISABLE_DEFAULT_CONFIG} to be <code>true</code>.
 * <p/>
 * If system property {@value #ENABLE_JMX} is set to <code>true</code>, when this class is initialized with a configuration, the configuration will also be
 * exposed to JMX via an instance of {@link BaseConfigMBean}, where you can update the properties via jconsole.
 * <p/>
 * Example:
 * 
 * <pre>
 ...
 * </pre>
 * <p/>
 * Please note that you should not cache the property value if you expect the value to change on-the-fly. For example, in the following code the change of the
...
public class DynamicPropertyFactory {
  ...
  /**
   * Boolean system property to define whether a configuration MBean should be registered with JMX so that properties can be accessed via JMX console. Default
   * is "unset".
   */
  public static final String ENABLE_JMX = "archaius.dynamicPropertyFactory.registerConfigWithJMX";

  /**
   * System property name that defines whether {@link #getInstance()} should throw {@link MissingConfigurationSourceException} if there is no proper
   * configuration source at the time of call.
   */
  ...
  private volatile static boolean throwMissingConfigurationSourceException = Boolean.getBoolean(THROW_MISSING_CONFIGURATION_SOURCE_EXCEPTION);
  ...
  /**
   * System property to determine whether DynamicPropertyFactory should be lazily initialized with default configuration for {@link #getInstance()}. Default is
   * false (not set).
   */
   ...
   * <p/>
   * The initialization will register a ConfigurationListener with the configuration so that {@link DynamicProperty} will receives a callback and refresh its
   * value when a property is changed in the configuration.
   * <p/>
   * If the factory is already initialized with a default configuration source (see {@link #getInstance()}), it will re-register itself with the new
   * configuration source passed to this method. Otherwise, this method will throw IllegalArgumentException if it has been initialized with a different and
   * non-default configuration source. This method should be only called once.
   *
   * @param config
   *          Configuration to be attached with DynamicProperty
   * @return the instance of DynamicPropertyFactory
   * @throws IllegalArgumentException
   *           if the factory has already been initialized with a non-default configuration source
   */
  ...
  /**
   * Get the backing configuration from the factory. This can be cased to a {@link ConcurrentCompositeConfiguration} if the default configuration is installed.
   * <p>
   * For example:
   * 
   * <pre>
   ...
   * </pre>
   *
   * @return the configuration source that DynamicPropertyFactory is initialized with. This will be an instance of Configuration, or
   *         {@link DynamicPropertySupport}, or null if DynamicPropertyFactory has not been initialized.
   */
  ...
  /**
   * Set the boolean value to indicate whether {@link #getInstance()} should throw {@link MissingConfigurationSourceException} if there is no proper
   * configuration source at the time of call.
   *
   * @param value
   *          to set
  ...
  /**
   * @return the boolean value to indicate whether {@link #getInstance()} should throw {@link MissingConfigurationSourceException} if there is no proper
   *         configuration source at the time of call.
   */
   ...
   * <p/>
   * The initialization will register a {@link PropertyListener} with the DynamicPropertySupport so that DynamicProperty will receives a callback and refresh
   * its value when a property is changed.
   * <p/>
   * If the factory is already initialized with a default configuration source (see {@link #getInstance()}), it will re-register itself with the new
   * configuration source passed to this method. Otherwise, this method will throw IllegalArgumentException if it has been initialized with a non-default
   * configuration source. This method should be only called once.
   *
   * @param dynamicPropertySupport
   *          DynamicPropertySupport to be associated with the DynamicProperty
   * @return the instance of DynamicPropertyFactory
   * @throws IllegalArgumentException
   *           if the factory has already been initialized with a different and non-default configuration source
   */
  ...
  /**
   * Get the instance to create dynamic properties. If the factory is not initialized with a configuration source (see
   * {@link #initWithConfigurationSource(AbstractConfiguration)} and {@link #initWithConfigurationSource(DynamicPropertySupport)}), it will fist try to
   * initialize itself with a default {@link ConcurrentCompositeConfiguration}, with the following two sub configurations:
   * <ul>
   * <li>A SystemConfiguration, which contains all the system properties. You can disable adding this Configuration by setting system property
   * {@value #DISABLE_DEFAULT_SYS_CONFIG} to <code>true</code>
   * <li>A {@link DynamicURLConfiguration}, which at a fixed interval polls a configuration file (see {@link URLConfigurationSource#DEFAULT_CONFIG_FILE_NAME})
   * on classpath and a set of URLs specified via a system property (see {@link URLConfigurationSource#CONFIG_URL}).
   * </ul>
   ...
   *          property name
   * @param defaultValue
   *          default value if the property is not defined in underlying configuration
   */
