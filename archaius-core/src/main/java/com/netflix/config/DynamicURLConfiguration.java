...
/**
 * A {@link DynamicConfiguration} that uses a {@link URLConfigurationSource} and {@link FixedDelayPollingScheduler}.
 * 
 * @author awang
 *
 */
public class DynamicURLConfiguration extends DynamicConfiguration {
  ...
  /**
   * Create an instance with default {@link URLConfigurationSource#URLConfigurationSource()} and {@link FixedDelayPollingScheduler#FixedDelayPollingScheduler()}
   * and start polling the source if there is any URLs available for polling.
   */
   ...
   * @param delayMillis
   *          delay interval in milliseconds used by {@link FixedDelayPollingScheduler}
   ...
   * @param urls
   *          The set of URLs to be polled by {@link URLConfigurationSource}
   */
  public DynamicURLConfiguration(int initialDelayMillis, int delayMillis, boolean ignoreDeletesFromSource, String... urls) {
    super(new URLConfigurationSource(urls), new FixedDelayPollingScheduler(initialDelayMillis, delayMillis, ignoreDeletesFromSource));
  }
}
