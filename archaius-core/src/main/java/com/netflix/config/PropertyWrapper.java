...
public abstract class PropertyWrapper<V> implements Property<V> {
  ...
  private static final IdentityHashMap<Class<? extends PropertyWrapper>, Object> SUBCLASSES_WITH_NO_CALLBACK = new IdentityHashMap<Class<? extends PropertyWrapper>, Object>();
  ...
  /**
   * By default, a subclass of PropertyWrapper will automatically register {@link #propertyChanged()} as a callback for property value change. This method
   * provide a way for a subclass to avoid this overhead if it is not interested to get callback.
   *
   * @param c
   */
  ...
  public String toString() {
    return "DynamicProperty: {name=" + prop.getName() + ", current value=" + prop.getString(String.valueOf(defaultValue)) + "}";
  }
}
