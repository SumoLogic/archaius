...
/**
 * Simple Class that has its field values influenced (assigned with values) from Properties Some fields can get initialized at startup only and NOT change when
 * actual property value is changed. Others can use <code>DynamicProperty</code> and contain values that are in sync with the runtime changes to the property
 * source.
 * 
 * @author stonse
 * 
...
class SampleBean {
  ...
  public SampleBean() {
    ...
    this.name = DynamicPropertyFactory.getInstance().getStringProperty("com.netflix.config.samples.SampleApp.SampleBean.name", "Sample Bean").get();
  }
  ...
  /**
   * Number of seeds in this bean. The value for this is automatically populated by the Configuration. Additionally, if the property value is modified, the new
   * value will be reflected as well We can assign a default value as well in case a value was not configured or the configuration was not successfully loaded
   * 
   */
  ...
  /**
   * This field utilizes the feature of Callbacks. When a property value is changed at runtime, it will receive a callback which can be used to trigger other
   * operations or used for bookkeeping
   */
  DynamicStringProperty sensitiveBeanData = DynamicPropertyFactory.getInstance()
    .getStringProperty("com.netflix.config.samples.SampleApp.SampleBean.sensitiveBeanData", "magic", new Runnable() {
      public void run() {
        // let my auditing system know about this change
        System.out.println("SampleBean.sensitiveData changed to:" + sensitiveBeanData.get());
      }
    });
