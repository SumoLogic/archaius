...
/**
 * A polled configuration source based on a set of URLs. For each poll, it always returns the complete union of properties defined in all files. If one property
 * is defined in more than one URL, the value in file later on the list will override the value in the previous one. The content of the URL should conform to
...
public class URLConfigurationSource implements PolledConfigurationSource {
  ...
  /**
   * Default configuration file name to be used by default constructor. This file should be on the classpath. The file name can be overridden by the value of
   * system property {@link #DEFAULT_CONFIG_FILE_PROPERTY}.
   */
  public static final String DEFAULT_CONFIG_FILE_NAME = "config.properties";

  /**
   * System property name to be used to learn the location of the configuration file name on the classpath.
   */
  ...
  public URLConfigurationSource() {
    ...
    if (urlList.size() == 0) {
      ...
      logger.info("To enable URLs as dynamic configuration sources, define System property " + CONFIG_URL + " or make " + DEFAULT_CONFIG_FILE_FROM_CLASSPATH
  ...
  /**
   * Retrieve the content of the property files. For each poll, it always returns the complete union of properties defined in all URLs. If one property is
   * defined in content of more than one URL, the value in file later on the list will override the value in the previous one.
   * 
   * @param initial
   ...
   *          this parameter is ignored by the implementation
   * @throws IOException
   *           IOException occurred in file operation
   */
  @Override
