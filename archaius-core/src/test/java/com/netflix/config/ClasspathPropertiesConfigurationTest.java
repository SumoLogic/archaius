...
public class ClasspathPropertiesConfigurationTest {
  ...
  /**
   * Default relative resource path is META-INF/conf/config.properties. All config files that mathc the pattern will be imported and loaded into the
   * configuration. In test/resource directory, there's a standalone META-INF/conf/config.properties file: standaloneConfigProperty = 10 There's another jar
   * file which has an embedded META-INF/conf/config.properties file: jarConfigProperty = 11 Once initialize() succeeds, both properties should be present in
   * config file.
   */
  @Test
