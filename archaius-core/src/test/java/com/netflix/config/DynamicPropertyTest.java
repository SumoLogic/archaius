...
public class DynamicPropertyTest {
  ...
  public void testDynamicProperty() {
    ...
    assertEquals("FastProperty does not have correct name", PROP_NAME, fastProp.getName());
    assertSame("DynamicProperty.getInstance did not find the object", fastProp, DynamicProperty.getInstance(PROP_NAME));
    //
    String hello = "Hello";
    assertNull("Unset DynamicProperty is not null", fastProp.getString());
    assertEquals("Unset DynamicProperty does not default correctly", hello, fastProp.getString(hello));
    config.setProperty(PROP_NAME, hello);
    assertEquals("Set DynamicProperty does not have correct value", hello, fastProp.getString());
    assertEquals("Set DynamicProperty uses supplied default", hello, fastProp.getString("not " + hello));
    assertEquals("Non-integer DynamicProperty doesn't default on integer fetch", 123, fastProp.getInteger(Integer.valueOf(123)).intValue());
    assertEquals("Non-float DynamicProperty doesn't default on float fetch", 2.71838f, fastProp.getFloat(Float.valueOf(2.71838f)).floatValue(), 0.001f);
    try {
      fastProp.getFloat();
    ...
    assertEquals("Set DynamicProperty does not have correct value", pi, fastProp.getString());
    assertEquals("DynamicProperty did not property parse float string", 3.14159f, fastProp.getFloat(Float.valueOf(0.0f)).floatValue(), 0.001f);
    config.setProperty(PROP_NAME, ee);
    assertEquals("Set DynamicProperty does not have correct value", ee, fastProp.getString());
    assertEquals("DynamicProperty did not property parse float string", 2.71838f, fastProp.getFloat(Float.valueOf(0.0f)).floatValue(), 0.001f);
    try {
      fastProp.getInteger();
    ...
    assertEquals("Integer fetch of non-integer DynamicProperty did not use default value", -123, fastProp.getInteger(Integer.valueOf(-123)).intValue());
    //
    String devil = "666";
    config.setProperty(PROP_NAME, devil);
    assertEquals("Changing DynamicProperty does not result in correct value", devil, fastProp.getString());
    assertEquals("Integer fetch of changed DynamicProperty did not return correct value", 666, fastProp.getInteger().intValue());
    //
    String self = "com.netflix.config.DynamicProperty";
    assertEquals("Fetch of named class from integer valued DynamicProperty did not use default", DynamicPropertyTest.class,
      fastProp.getNamedClass(DynamicPropertyTest.class));
    config.setProperty(PROP_NAME, self);
    assertEquals("Fetch of named class from DynamicProperty did not find the class", DynamicProperty.class, fastProp.getNamedClass());
    // Check that clearing a property clears all caches
    config.clearProperty(PROP_NAME);
    assertNull("Fetch of cleard property did not return null", fastProp.getString());
    assertEquals("Fetch of cleard property did not use default value", devil, fastProp.getString(devil));
    assertNull("Fetch of cleard property did not return null", fastProp.getInteger());
    assertEquals("Fetch of cleard property did not use default value", -123, fastProp.getInteger(Integer.valueOf(-123)).intValue());
    assertNull("Fetch of cleard property did not return null", fastProp.getFloat());
    assertEquals("Fetch of cleard property did not use default value", 2.71838f, fastProp.getFloat(Float.valueOf(2.71838f)).floatValue(), 0.001f);
    assertNull("Fetch of cleard property did not return null", fastProp.getNamedClass());
    assertEquals("Fetch of cleard property did not use default value", DynamicProperty.class, fastProp.getNamedClass(DynamicProperty.class));
    //
    String yes = "yes";
    String maybe = "maybe";
    String no = "Off";
    config.setProperty(PROP_NAME, yes);
    assertTrue("boolean property set to 'yes' is not true", fastProp.getBoolean().booleanValue());
    config.setProperty(PROP_NAME, no);
    assertTrue("boolean property set to 'no' is not false", !fastProp.getBoolean().booleanValue());
  ...
  public void testDynamicProperySetAdnGets() throws Exception {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      config.setProperty("com.netflix.testing.mybool", "true");
      assertTrue(prop.get());
      assertTrue(config.getString("com.netflix.testing.mybool").equals("true"));
      config.setProperty("com.netflix.testing.mybool", "false");
      assertFalse(prop.get());
    ...
    for (int i = 0; i < 100; i++) {
      ...
      assertTrue(prop.get());
      assertTrue(config.getString("com.netflix.testing.mybool").equals("true"));
      config.clearProperty("com.netflix.testing.mybool");
      assertFalse(prop.get());
